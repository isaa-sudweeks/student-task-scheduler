...
interface CourseMembersDialogProps {
  courseId: string;
  courseTitle: string;
  open: boolean;
  onClose: () => void;
...
export function CourseMembersDialog({
  ...
  const membersQuery = api.course.members.useQuery(
    { courseId },
    { enabled: open },
  );

  const invite = api.course.inviteMember.useMutation({
    onSuccess: async () => {
      toast.success("Collaborator added");
      await utils.course.members.invalidate({ courseId });
      setEmail("");
  ...
  const updateRole = api.course.updateMemberRole.useMutation({
    onSuccess: async () => {
  ...
  const removeMember = api.course.removeMember.useMutation({
    onSuccess: async () => {
  ...
  return (
    <Modal open={open} onClose={onClose} title={`Share “${courseTitle}”`} footer={footer}>
      <div className="space-y-4">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          removeMember.mutate({ courseId, userId: member.userId })
                        }
                      >
                        Remove
                      </Button>
        ...
        <div className="space-y-2 border-t pt-3">
          ...
          <div className="space-y-1">
            <label htmlFor="course-invite-email" className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
              Email
            </label>
            <Input
          ...
          <div className="space-y-1">
            <label htmlFor="course-invite-role" className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
              Role
            </label>
            <select
              id="course-invite-role"
              value={role}
          ...
          <Button
            ...
            onClick={() =>
              invite.mutate({ courseId, email, role })
            }
          >
...
export default CourseMembersDialog;
